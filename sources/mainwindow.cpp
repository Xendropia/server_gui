    ...
    m_userName(userName)
{
    ...
    // Initialize user table view for banning/disconnecting
    userTableView = new QTableView(this);
    userTableView->setModel(new QStandardItemModel());  // Model for user list

    // Initialize the ban and disconnect buttons
    banButton = new QPushButton("Ban User", this);
    disconnectButton = new QPushButton("Disconnect User", this);

    connect(banButton, &QPushButton::clicked, this, &MainWindow::onBanUser);
    connect(disconnectButton, &QPushButton::clicked, this, &MainWindow::onDisconnectUser);
...
void MainWindow::banUserById(int userId)
{
    // Logic to ban the user from the database
    m_dbPtr->banUser(userId);
}

// Implement backend logic to disconnect a user
void MainWindow::disconnectUserById(int userId)
{
    // Logic to disconnect the user from the session
    m_dbPtr->disconnectUser(userId);
}
...
// New slot for banning a user
void MainWindow::onBanUser()
{
    QModelIndexList selected = userTableView->selectionModel()->selectedRows();
    if (!selected.isEmpty()) {
        int userId = selected.at(0).data().toInt();
        banUserById(userId);
    }
}

// New slot for disconnecting a user
...
// Implement backend logic to ban a user
